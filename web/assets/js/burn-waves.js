...
(function () {
  ...
  el("preview-form").addEventListener("submit", async (e) => {
    e.preventDefault();
    const address = el("preview-input").value.trim();
    if (!address) return;
    const err = el("preview-error");
    err.hidden = true;
    el("preview-result").hidden = true;
    el("preview-go").disabled = true;
    const res = await apiGet("/api/preview/waves/" + encodeURIComponent(address));
  ...
  async function initBind() {
    ...
    if (params.get("hearth")) el("bind-hearth").value = params.get("hearth");
